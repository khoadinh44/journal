...
def extracted_model(in_, opt):
  ...
  soft_out = Dense(opt.num_classes, activation='softmax')(x)
...
def train_new_triplet_center(opt, x_train_scale, x_train, y_train, x_test_scale, x_test, y_test, network, i=100):
    ...
    extract_model_1, extract_model_1_s  = extracted_model(extract_input_1, opt)
    ...
    extract_model_2, extract_model_2_s  = extracted_model(extract_input_2, opt)
    ...
    extract_model_3, extract_model_3_s  = extracted_model(extract_input_3, opt)
    ...
    extract_model_s = concatenate([extract_model_1_s, extract_model_2_s, extract_model_3_s], axis=-1)
    ...
    # Fit data-------------------------------------------------
    model = Model(inputs=[anchor_input, extract_input_1, positive_input, extract_input_2, negative_input, extract_input_3, target_input], outputs=[merged_soft, extract_model_s, merged_pre])
    if opt.use_weight:
    ...
    model.fit(x=[anchor, anchor_extract, positive, positive_extract, negative, negative_extract, y_target], y=[target, target, y_target],
